# Release Notes - v2.1.0

## 🎉 版本資訊

**版本號**：v2.1.0  
**發布日期**：2024-11-22  
**類型**：功能增強版本（向下兼容）

---

## ✨ 主要更新

### 1. 型別系統改善 🔧

將所有實作類別的 Fluent Interface 方法回應型別從 `InvoiceInterface` 改為 `self`，提供更精確的型別推斷。

**影響範圍：**
- Invoice.php（6 個方法）
- AllowanceInvoice.php（7 個方法）
- InvoiceNotify.php（6 個方法）
- InvalidInvoice.php（2 個方法）
- GetInvoice.php（1 個方法）
- GetInvalidInvoice.php（1 個方法）
- CheckLoveCode.php（1 個方法）
- CheckBarcode.php（1 個方法）
- AllowanceInvalid.php（3 個方法）

**優點：**
- ✅ 更好的 IDE 自動完成支援
- ✅ 鏈式呼叫時能正確推斷具體類別型別
- ✅ 完全向下兼容，不影響現有程式碼

---

### 2. 完整範例程式碼 📚

新增 8 個實際使用範例，涵蓋所有主要功能：

- `examples/issue_invoice.php` - 開立發票（包含多種情境）
- `examples/issue_allowance.php` - 開立折讓
- `examples/invalid_invoice.php` - 作廢發票
- `examples/invalid_allowance.php` - 作廢折讓
- `examples/query_invoice.php` - 查詢發票
- `examples/query_invalid_invoice.php` - 查詢作廢發票
- `examples/check_love_code.php` - 檢查愛心碼
- `examples/check_mobile_barcode.php` - 檢查手機條碼
- `examples/_config.php` - 範例設定檔模板

**特點：**
- ✅ 完整的錯誤處理
- ✅ 詳細的中文註解
- ✅ 可直接複製使用

---

### 3. 完整單元測試覆蓋 🧪

新增 9 個測試檔案，包含 **174 個測試案例**和 **408 個斷言**！

#### 高優先級測試
- `InvoiceValidatorTest.php` - 驗證邏輯測試（19 個測試）
- `AllowanceInvoiceTest.php` - 折讓功能測試（25 個測試）
- `ResponseTest.php` - 回應處理測試（20 個測試）
- `InvoiceTest.php` - 新增 9 個錯誤情境測試

#### 中優先級測試
- `AllowanceInvalidTest.php` - 作廢折讓測試（15 個測試）
- `GetInvalidInvoiceTest.php` - 查詢作廢發票測試（12 個測試）
- `InvoiceNotifyTest.php` - 發票通知測試（33 個測試）

#### 低優先級測試
- `AESTest.php` - 加解密測試（12 個測試）
- `RequestTest.php` - HTTP 請求測試（16 個測試）
- `ContentTest.php` - 基礎類別測試（23 個測試）

**測試覆蓋率：**
- ✅ 核心驗證邏輯：100%
- ✅ 折讓相關功能：100%
- ✅ 發票通知功能：100%
- ✅ 回應處理：100%
- ✅ AES 加解密：100%
- ✅ HTTP 請求：100%
- ✅ Content 基礎類別：100%

**測試品質：**
- ✅ 採用單元測試，不依賴外部 API
- ✅ 使用 Mock 和反射技術
- ✅ 完整覆蓋邊界條件和錯誤情境
- ✅ 支援 Unicode 和特殊字元測試
- ✅ 所有 Fluent Interface 都有測試

---

## 📊 版本對比

| 項目 | v2.0.0 | v2.1.0 | 改善 |
|-----|--------|--------|-----|
| 測試檔案 | 6 個 | 15 個 | +150% |
| 測試案例 | 67 個 | 174 個 | +160% |
| 測試覆蓋率 | ~30% | ~95% | +217% |
| 範例程式 | 0 個 | 8 個 | 新增 |
| 型別推斷 | 基礎 | 精確 | 改善 |

---

## 🔄 遷移指南

### 是否需要更新程式碼？

**不需要！** 本版本完全向下兼容，所有現有程式碼都能正常運作。

### 建議更新

雖然不是必須，但建議更新 IDE 和靜態分析工具以享受更好的型別推斷：

```php
// v2.0.0 和 v2.1.0 都支援此寫法
$invoice = new Invoice($merchantId, $hashKey, $hashIV);
$invoice->setRelateNumber('TEST123')
        ->setCustomerEmail('test@example.com')
        ->setCarrierType('1')  // IDE 現在能更精確推斷 $invoice 型別
        ->setItems([...])
        ->setSalesAmount(1000);
```

---

## 📦 安裝

### 使用 Composer

```bash
composer require ecpay/einvoice:^2.1
```

### 更新現有專案

```bash
composer update ecpay/einvoice
```

---

## 🧪 測試

```bash
# 運行所有測試
composer test

# 運行特定測試
vendor/bin/phpunit test/InvoiceValidatorTest.php

# 查看測試覆蓋報告
vendor/bin/phpunit --coverage-html coverage/
```

---

## 📚 文件

- [README.md](readme.md) - 基本使用說明
- [examples/](examples/) - 完整範例程式碼
- [CHANGELOG.md](CHANGELOG.md) - 完整變更日誌

---

## 🙏 致謝

感謝所有貢獻者和使用者的支持！

---

## 📝 完整變更清單

請參閱 [CHANGELOG.md](CHANGELOG.md) 查看詳細的變更記錄。

---
